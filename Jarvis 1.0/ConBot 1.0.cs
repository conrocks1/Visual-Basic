...
namespace ConBot
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            cpuMaxedOutMessages.Add("WARNING: Stop downloading the crap it's maxing me out");
            ...
            synth.Speak("Welcome to Con Bot version one point oh!");
            ...
            ConSpeak(systemUptimeMessage, VoiceGender.Male, 2);
            ...
            while(true)
            {
                ...
                if ( currentCpuPercentage > 80 )
                {
                    if (currentCpuPercentage == 100)
                    {
                        ...
                        ConSpeak(cpuLoadVocalMessage, VoiceGender.Male, speechSpeed);
                    }
                    else
                    {
                        string cpuLoadVocalMessage = String.Format("The current CPU load is {0} percent", currentCpuPercentage);
                        ConSpeak(cpuLoadVocalMessage, VoiceGender.Female, 5);
                    }
                }
                #endregion
                ...
                if (currentAvailableMemory < 1024)
                {
                    ...
                    ConSpeak(memAvailableVocalMessage, VoiceGender.Male, 5);
                }
        ...
        public static void ConSpeak(string message, VoiceGender voiceGender)
        {
        ...
        public static void ConSpeak(string message, VoiceGender voiceGender, int rate)
        {
            synth.Rate = rate;
            ConSpeak(message, voiceGender);
        }
